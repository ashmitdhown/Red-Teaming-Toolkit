...
export const TelemetryInspector = () => {
  const { metrics, baselinePosProb } = useAppContext();
    ...
    <div className="h-56 bg-surface-panel relative flex flex-col shrink-0">
      ...
      <div className="flex-1 p-5 flex flex-col justify-center">
        <div className="flex justify-between items-end border-b border-ui-border/50 pb-3 mb-4">
          ...
          <span className={`font-mono text-3xl font-bold tracking-tighter ${jsdValue > 0.1 ? 'glitch-active' : ''}`}>
            {metrics.jsd}
          </span>
        </div>
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <span className="font-mono text-sm text-ui-text font-semibold">
              {baselinePosProb ? `${(baselinePosProb * 100).toFixed(1)}% CONF` : 'UNTRIED'}
            </span>
          </div>
        ...
        <div className="mt-5 h-1.5 w-full bg-surface-alt relative overflow-hidden rounded-full">
          <div className="absolute right-0 top-0 bottom-0 w-[90%] bg-ui-alert/10 border-l-2 border-ui-alert/50"></div>
          <div 
            className="absolute left-0 top-0 bottom-0 bg-ui-text transition-all duration-300 rounded-full"
            style={{ width: `${percent}%`, backgroundColor: jsdValue > 0.1 ? 'rgb(var(--col-alert))' : 'rgb(var(--col-text))' }}
          ></div>
        </div>
        <div className="flex justify-between mt-2">
          <span className="font-mono text-[10px] text-ui-muted">0.0</span>
          <span className="font-mono text-[10px] font-semibold text-ui-alert">THRESHOLD: 0.10</span>
        </div>
      </div>
    </div>
  );
