...
export const RobustnessFingerprint = () => {
  const { isGhost } = useAppContext();

  const points = isGhost 
    ? "70,29 109.4,47.25 75.2,73 70,100.5 39.26,87.75 34.06,49.25"
    : "70,29 109.4,47.25 90.78,82 70,100.5 39.26,87.75 34.06,49.25";
  
  const encDotCx = isGhost ? "75.2" : "90.78";
  const encDotCy = isGhost ? "73" : "82";
  
  const encScore = isGhost ? 12 : 48;
  const overallRating = isGhost ? "58/100" : "67/100";
    ...
    <div className="flex-1 bg-surface-panel relative flex flex-col overflow-hidden">
      ...
      <div className="flex-1 p-4 font-mono text-xs text-ui-text overflow-auto selectable">
        ...
        <div className="flex flex-row gap-5 mb-5 items-center border-b border-ui-border/50 pb-5">
          <div className="w-[140px] h-[140px] shrink-0 relative">
            <svg viewBox="0 0 140 140" className="w-full h-full font-mono text-[8px] tracking-widest overflow-visible">
              ...
              <text x="117" y="97" fill="currentColor" textAnchor="start" className={`transition-colors font-bold ${isGhost ? 'text-ui-alert' : 'text-ui-muted'}`}>ENC</text>
              ...
              <polygon 
                points={points} 
                fill={isGhost ? "rgb(var(--col-alert))" : "rgb(var(--col-text))"} 
                fillOpacity="0.15" 
                stroke={isGhost ? "rgb(var(--col-alert))" : "rgb(var(--col-text))"} 
                strokeWidth="1.5" 
        ...
        <div className="space-y-2.5 text-[11px]">
          <ScoreItem label="Input Validation" score={82} />
          <ScoreItem label="Boundary Handling" score={91} />
          
          <div className={`flex items-center justify-between group transition-colors ${isGhost ? 'row-active-alert' : ''}`}>
            <span className={`w-32 truncate transition-colors ${isGhost ? 'text-ui-alert' : 'group-hover:text-ui-alert'}`}>Encoding Robustness</span>
            <div className="flex-1 flex h-[8px] mx-2 bg-surface-base border border-ui-border/30 transition-all duration-500">
              <div className={`transition-all duration-500 ${isGhost ? 'bg-ui-alert' : 'bg-ui-text'}`} style={{ width: `${encScore}%` }}></div>
            ...
            <span className={`w-6 text-right font-bold transition-colors ${isGhost ? 'text-ui-alert' : ''}`}>{encScore}</span>
          </div>

          <ScoreItem label="Perturbation Stability" score={61} />
          <ScoreItem label="Prompt Safety" score={71} />
          <ScoreItem label="Error Handling" score={83} />
        </div>
        ...
        <div className="mt-4 pt-3 border-t border-ui-border flex justify-between items-end">
          ...
          <div className="text-right">
            ...
            <span className={`text-[10px] ml-2 font-bold ${isGhost ? 'text-ui-alert animate-pulse' : 'text-ui-text'}`}>{riskLevel}</span>
          </div>
        </div>
